...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    const gapPercentage = isMobile ? 0.1 : 0.15; 
    this.padding = isMobile ? 1.5 : planeWidth * gapPercentage;
...
const CircularGallery = forwardRef(({
  ...
  const containerRef = useRef(null);
  // CHANGED: Use useRef instead of useState for the App instance
  ...
  useEffect(() => {
    ...
    const newApp = new App(containerRef.current, { items, bend, borderRadius, scrollSpeed, scrollEase });
    appRef.current = newApp; // Store synchronous reference
  ...
  // CHANGED: Access the app via the Ref, not State
  useImperativeHandle(ref, () => ({
    moveLeft: () => {
      if (appRef.current) appRef.current.move(-1); 
    },
    moveRight: () => {
      if (appRef.current) appRef.current.move(1); 
    }
